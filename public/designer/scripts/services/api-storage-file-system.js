...
//  .factory('apiStorageFileSystem', function ($http, $q, $timeout, localStorageHelper, FOLDER) {
    ...
    // mongodb uri
//    var host = '../../conf/conf'.HOST;
//    var host = 'localhost:5000/v2';
    var host = 'raml-store.herokuapp.com';
...
//      console.log("service.createFolder... " + path);
      var deferred = $q.defer();
      var file = {};
...
//      console.log("++++ " + files[path]);
      $http({
          method: 'GET',
          data: '',
          url: 'http://' + host + '/files/' + files[path],
          withCredentials: false
        }).success(function (data) {
    ...
    service.remove = function (path) {
      var deferred = $q.defer();
      console.log("service.remove... " + path);
      if (!files[path]) {
    ...
    service.rename = function (source, destination) {
      console.log("service.rename... " + source + " - " + destination);
