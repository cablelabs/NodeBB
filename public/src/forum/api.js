...
define('forum/api', ['composer', 'forum/pagination', 'forum/infinitescroll', 'share', 'navigator', 'forum/categoryTools'], function(composer, pagination, infinitescroll, share, navigator, categoryTools) {
    var Api = {};

    $(window).on('action:ajaxify.start', function(ev, data) {
        if(data && data.url.indexOf('api') !== 0) {
            navigator.hide();
...
//        socket.removeListener('event:new_topic', Api.onNewTopic);
//        socket.addListener
...
//    }
    ...
    Api.init = function() {
        ...
        socket.on('event:new_topic', Api.onNewTopic);
        ...
        if (!config.usePagination) {
            navigator.init('#topics-container > .category-item', ajaxify.variables.get('topic_count'), undefined, Api.toTop, Api.toBottom);
        }
                ...
                if(config.usePagination) {
                    socket.emit('topics.getTidPage', bookmark, function(err, page) {
                        ...
                        if(parseInt(page, 10) !== pagination.currentPage) {
                            ...
                            Api.scrollToTopic(bookmark, clicked, 400);
                        }
                    });
                } else {
                    ...
                    socket.emit('topics.getTidIndex', bookmark, function(err, index) {
                        ...
                        loadTopicsAfter(index, function() {
                            Api.scrollToTopic(bookmark, clicked, 0);
                        });
                    });
                }
            }
    ...
    Api.highlightTopic = function(tid) {
    ...
    Api.scrollToTopic = function(tid, clickedTid, duration, offset) {
        if(!tid) {
            return;
        }
    ...
    function enableInfiniteLoadingOrPagination() {
        if (!config.usePagination) {
            infinitescroll.init(Api.loadMoreTopics);
        } else {
            navigator.hide();
    ...
    Api.onTopicsLoaded = function(data, callback) {
    ...
    Api.loadMoreTopics = function(direction) {
        ...
        infinitescroll.calculateAfter(direction, '#topics-container .category-item[data-tid]', config.topicsPerPage, function(after, offset, el) {
            ...
            loadTopicsAfter(after, function() {
                if (direction < 0 && el) {
                    Api.scrollToTopic(el.attr('data-tid'), null, 0, offset);
                }
            });
        });
    };
    ...
    function loadTopicsAfter(after, callback) {
        ...
        infinitescroll.loadMore('categories.loadMore', {
            ...
            if (data.topics && data.topics.length) {
                Api.onTopicsLoaded(data, function() {
                    done();
                    callback();
                });
